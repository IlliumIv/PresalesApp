...
namespace PresalesApp.Database.Helpers
{
    ...
    {
        private static readonly Dictionary<int, byte[,]> DaysByType = [];
        ...
        public static double CalculateBusinessMinutesLOCAL(this DateTime start, DateTime end)
        {
            ...
            start = start.TimeOfDay == new DateTime().TimeOfDay ? start : start + timeZoneOffset;
            end = end.TimeOfDay == new DateTime().TimeOfDay ? end : end + timeZoneOffset;
            ...
            if (end.Date - start.Date > TimeSpan.FromDays(1))
            {
                ...
                foreach (DateTime dt in EachDay(start.AddDays(1), end.AddDays(-1)))
                {
        ...
        private static void LoadCalendar(int year)
        {
            try
            {
                if (!DaysByType.ContainsKey(year))
                {
                    ...
                    if (!File.Exists(path))
                    {
                        DownloadXmlCalendar(year, path);
                    }
                    ParseXmlCalendar(path);
                }
            }
            catch { }
        }
        ...
        {
            LoadCalendar(dt.Year);

            var day_type = DaysByType.TryGetValue(dt.Year, out byte[,]? value) ? value?[dt.Month - 1, dt.Day - 1] : 0;
            ...
            if (start_of_period > end_of_day) return 0;

            return day_type switch
            {
                // 1 - выходной день, 2 - рабочий и сокращенный, 3 - рабочий день (суббота/воскресенье)
                0 => dt.DayOfWeek == DayOfWeek.Saturday || dt.DayOfWeek == DayOfWeek.Sunday ? 0 : ((TimeSpan)end_of_day - start_of_period).TotalMinutes,
                1 => 0,
        ...
        private static void ParseXmlCalendar(string path)
        {
            ...
            DaysByType.Add(year, new byte[12, 31]);
            ...
            foreach (XmlNode node in days)
            {
                ...
                DaysByType[year][month - 1, day - 1] = value;
            }
        }

        private static void DownloadXmlCalendar(int year, string path)
        {
