...
public static class ChartHelpers
{
    ...
    public static ChartJsConfig GenerateChartConfig(ChartType chartType, IList<string>? labels = null,
    ...
    public static LineDataset GetLineDataset(IList<object> data, string label,
        string? color = null, float? backgroundAlfa = null, float? borderAlfa = null, bool fill = true, double pointRadius = 1) => new()
        {
            Label = label,
            Data = data,
            BackgroundColor = color is null ? null : GetColor(color, backgroundAlfa ?? _BackgroundAlfa),
            BorderColor = color is null ? null : GetColor(color, borderAlfa ?? _BorderAlfa),
            Fill = fill,
            PointRadius = pointRadius
        };

    public static PieDataset GetPieDataset(IList<object> data, (byte R, byte G, byte B)? lastColor = null,
        float? backgroundAlfa = null, float? borderAlfa = null, StringOrDoubleValue? cutout = null) => new()
        {
            Data = data,
            BackgroundColor = GetColors((ushort)data.Count, backgroundAlfa ?? _BackgroundAlfa, lastColor),
    ...
    public static List<string> GetColors(ushort count, float alfa, (byte R, byte G, byte B)? last = null)
    {
        ...
        {
            res.Remove(res.Last());
            res.Add($"rgba({last.Value.R}, {last.Value.G}, {last.Value.B}, {alfa.ToStringInvariant()})");
        }

        return res;
    }

    public static string GetColor(string color, float alfa) => Colors.ContainsKey(color) switch
    {
        true => $"rgba({Colors[color].R}, {Colors[color].G}, {Colors[color].B}, {alfa.ToStringInvariant()})",
        false => color
    };
    ...
    public static string GetColorAt(ushort index, float alfa)
    {
    ...
    public static void Update(this ChartJsConfig config, IList<string>? labels = null, params ChartJsDataset[] datasets)
    {
        config.SetLabels(labels ?? _GenerateEmptyStringList(datasets.MaxBy(ds => ds.Data.Count)?.Data.Count ?? 0));
